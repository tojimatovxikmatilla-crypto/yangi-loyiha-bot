...
# Diqqat: StateFilter(None) olib tashlandi — shunda bu tugma foydalanuvchi
# boshqa funksiyada (masalan Ob-havo shahar kutayotganda) turgan bo'lsa ham
# ishlaydi va avvalgi funksiyani yopib, Shazam'ni ochadi.
...
async def handle_shazam_audio(message: Message, state: FSMContext, bot: Bot):
    ...
    if not result.success:
        ...
        # state.clear() o'rniga qayta ovoz kutish holatiga qaytaramiz — foydalanuvchi
        # tugmani qayta bosmasdan keyingi ovozli xabarni to'g'ridan-to'g'ri yuborishi mumkin.
        await state.set_state(ShazamStates.waiting_for_audio)
        return
    ...
    # state.clear() o'rniga qayta ovoz kutish holatiga qaytaramiz — foydalanuvchi
    # tugmani qayta bosmasdan keyingi qo'shiqni to'g'ridan-to'g'ri yuborib,
    # Shazam'dan ketma-ket foydalanishi mumkin.
=======
import os
import uuid
import asyncio
