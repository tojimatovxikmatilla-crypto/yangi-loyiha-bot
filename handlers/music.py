...
def _build_page_text_and_kb(query: str, results: list, page: int):
    ...
    kb.adjust(5)  # 10 ta tugmani 5+5 qilib ikki qatorga joylaydi (bitta qatorda max 8 bo'lishi kerak)
...
# handler uni "qidiruv so'zi" deb qabul qilib olmasligi va xabar tegishli
# tugmaning o'z handleriga o'tishi uchun.
...
async def handle_music_query(message: Message, state: FSMContext, bot):
    ...
    # Holatni tozalamaymiz — shunda foydalanuvchi tugmani qayta bosmasdan,
    # to'g'ridan-to'g'ri keyingi qo'shiq nomini yozib davom etaveradi.
    await state.set_state(MusicStates.waiting_for_query)
